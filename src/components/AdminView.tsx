...
export default function AdminView({ rate, config, onUpdateRate, onUpdateConfig, currentUser }: AdminViewProps) {
  ...
  const handleSaveProduct = async (e: FormEvent) => {
    ...
    try {
      const payload: Producto = {
        ...
        imagen_url: prodImagenUrl.trim() || undefined
      };
  ...
  const handleCreateUserCode = async (e: FormEvent) => {
    e.preventDefault();
    if (!userNombre.trim()) return;

    const normalizedNewName = normalizeName(userNombre);
    const isUserNameDuplicate = usuarios.some(u => u.id !== editUserId && normalizeName(u.nombre) === normalizedNewName);
    ...
    try {
      if (editUserId) {
        ...
        const updates: Partial<UsuarioAcceso> = {
          nombre: userNombre.trim(),
          rol: userRol
        };
        
        // If a new code was generated, hash and update it
        if (generatedCode) {
          const codeHash = await hashString(generatedCode);
          updates.código_acceso_cifrado = codeHash;
          alert(`Código actualizado para ${userNombre}: "${generatedCode}". Copia este código ahora, ya que no se podrá recuperar de la base de datos (se almacena cifrado de forma segura).`);
        } else {
          alert(`Nombre de usuario actualizado para ${userNombre} correctamente.`);
        }

        await dbService.updateItem('usuarios_acceso', editUserId, updates);
      } else {
        // Creating a new user
        if (!generatedCode) {
          alert('Por favor, genere un código de acceso.');
          setSavingUser(false);
          return;
        }
        const codeHash = await hashString(generatedCode);
  ...
  const editUser = (u: UsuarioAcceso) => {
    setEditUserId(u.id || '');
    setUserNombre(u.nombre);
    setUserRol(u.rol as 'staff' | 'vip');
    setGeneratedCode('');
    ...
    <div id="admin-view" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
              <form onSubmit={handleSaveProduct} className="bg-white rounded-3xl p-6 max-w-md w-full border border-orange-100 shadow-2xl animate-scaleUp space-y-4">
                ...
                <div className="border-t border-slate-100 pt-3 space-y-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">Imagen de la Pizza</label>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`flex-1 py-1 px-3 rounded-lg text-xs font-semibold border transition cursor-pointer text-center ${
                        imageType === 'url'
                    ...
                    <div className="space-y-2">
                      <input
                        ...
                        onChange={async (e) => {
                          ...
                          setIsUploading(true);
                          try {
                            const uploadedUrl = await dbService.uploadFile(
                              file,
                              `productos/${Date.now()}_${file.name.replace(/\s+/g, '_')}`
                            );
                            setProdImagenUrl(uploadedUrl);
                      ...
                      {isUploading && (
                        <div className="text-[10px] text-orange-600 font-bold animate-pulse">
                          Subiendo imagen a Firebase Storage... por favor espera.
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {prodImagenUrl && (
                    <div className="relative mt-2 w-full h-24 rounded-lg overflow-hidden border border-slate-200 bg-slate-50">
                      <img src={prodImagenUrl} alt="Vista previa" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                      <button
                        type="button"
                        onClick={() => setProdImagenUrl('')}
                        className="absolute top-1 right-1 bg-red-500 hover:bg-red-600 text-white p-1 rounded-full shadow cursor-pointer"
                        title="Quitar imagen"
                      >
                      ...
                      <tr key={p.id}>
                        <td className="px-6 py-4 font-bold text-slate-900">
                          <div className="flex items-center gap-3">
                            {p.imagen_url ? (
                              <img src={p.imagen_url} alt={p.nombre} className="w-10 h-10 rounded-lg object-cover bg-slate-50 border border-slate-150 shrink-0" referrerPolicy="no-referrer" />
                            ) : (
                              <div className="w-10 h-10 rounded-lg bg-slate-100 border border-slate-150 flex items-center justify-center text-slate-400 shrink-0">
                        ...
                        <td className="px-6 py-4 text-right space-x-2">
                          <button
                            ...
                            className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold py-1.5 px-3 rounded-lg cursor-pointer transition"
                          >
                            Editar
                          </button>
                          <button
                            onClick={() => deleteProduct(p.id!)}
                            className="bg-red-50 hover:bg-red-100 text-red-600 font-semibold py-1.5 px-3 rounded-lg cursor-pointer transition"
                          >
                            Eliminar
                          </button>
                        </td>
                  ...
                  <div key={p.id} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm space-y-3">
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-2.5">
                        {p.imagen_url ? (
                          <img src={p.imagen_url} alt={p.nombre} className="w-12 h-12 rounded-xl object-cover bg-slate-50 border border-slate-150 shrink-0" referrerPolicy="no-referrer" />
                        ) : (
                          <div className="w-12 h-12 rounded-xl bg-slate-100 border border-slate-150 flex items-center justify-center text-slate-400 shrink-0">
                      ...
                      <button
                        onClick={() => toggleProductDispo(p)}
                        className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md font-semibold text-[10px] transition cursor-pointer ${
                          p.disponibilidad
                            ? 'bg-emerald-100 text-emerald-800'
                            : 'bg-red-100 text-red-800'
                        }`}
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs border-t border-slate-100 pt-3 font-mono">
                      <div>
                        <span className="text-[9px] text-slate-400 font-sans font-bold block uppercase tracking-wider">Familiar (30cm)</span>
                        <span className="font-bold text-slate-900 block">{formatCUP(p.precio_familiar_cup)}</span>
                        <span className="text-[10px] text-slate-500 block">{formatUSD(p.precio_familiar_cup, rate)}</span>
                      </div>
                      <div>
                        <span className="text-[9px] text-slate-400 font-sans font-bold block uppercase tracking-wider">XXL (40cm)</span>
                        <span className="font-bold text-slate-900 block">{formatCUP(p.precio_xxl_cup)}</span>
                        <span className="text-[10px] text-slate-500 block">{formatUSD(p.precio_xxl_cup, rate)}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2 border-t border-slate-100 pt-3">
                      <button
                        onClick={() => editProduct(p)}
                        className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-2 rounded-xl text-xs transition cursor-pointer text-center"
                      >
                        Editar
                      </button>
                      <button
                        onClick={() => deleteProduct(p.id!)}
                        className="flex-1 bg-red-50 hover:bg-red-100 text-red-600 font-bold py-2 rounded-xl text-xs transition cursor-pointer text-center"
                      >
                        Eliminar
                      </button>
                    </div>
                ...
                {cupones.map((c) => (
                  <div key={c.id} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-slate-900 tracking-wider font-mono text-sm">{c.código}</span>
                      <button
                        onClick={() => toggleCouponVigente(c)}
                        className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md font-semibold text-[10px] transition cursor-pointer ${
                          c.vigencia
      ...
      {activeTab === 'usuarios' && isCeo && (
        <div id="tab-usuarios" className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            {staffList.length === 0 ? (
              ...
              <div className="space-y-4">
                          ...
                          <td className="px-6 py-4 text-right space-x-2">
                            ...
                            {u.activo ? (
                              <button
                                ...
                                className="bg-amber-50 hover:bg-amber-100 text-amber-700 font-semibold py-1.5 px-3 rounded-lg cursor-pointer transition"
                              >
                                Revocar
                              </button>
                            ) : (
                              <button
                                ...
                                className="bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-semibold py-1.5 px-3 rounded-lg cursor-pointer transition"
                              >
                                Reactivar
                              </button>
                            )}
                ...
                <div className="grid grid-cols-1 gap-4 md:hidden">
                  {staffList.map((u) => (
                    <div key={u.id} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm space-y-3">
                      <div className="flex justify-between items-center">
                        <h4 className="font-bold text-slate-900 text-sm">{u.nombre}</h4>
                        <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full font-bold text-[10px] ${
                          u.activo ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800'
                        }`}>
                          {u.activo ? 'Activo' : 'Revocado'}
                      ...
                      <div className="text-xs text-slate-500 font-mono flex justify-between">
                        <span>Rol: Staff Cocina</span>
                        <span className="text-slate-400 italic">[Cifrado SHA-256]</span>
                      </div>
                      ...
                      <div className="flex gap-2 border-t border-slate-100 pt-3">
                        ...
                        {u.activo ? (
                          <button
                            onClick={() => revokeUserAccess(u.id!)}
                            className="flex-1 bg-amber-50 hover:bg-amber-100 text-amber-700 font-bold py-2 rounded-xl text-xs transition cursor-pointer text-center"
                          >
                            Revocar
                          </button>
                        ) : (
                          <button
                            onClick={() => reactivateUserAccess(u.id!)}
                            className="flex-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold py-2 rounded-xl text-xs transition cursor-pointer text-center"
                          >
                            Reactivar
                          </button>
                        )}
          ...
          {showUserForm && (() => {
            const normalizedNewName = normalizeName(userNombre);
            const isUserNameDuplicate = userNombre.trim() !== '' && usuarios.some(u => u.id !== editUserId && normalizeName(u.nombre) === normalizedNewName);
            ...
            return (
              <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                <form onSubmit={handleCreateUserCode} className="bg-white rounded-3xl p-6 max-w-sm w-full border border-orange-100 shadow-2xl animate-scaleUp space-y-4">
                  <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                    <h4 className="font-bold text-slate-900 text-base">{editUserId ? 'Editar Credenciales' : 'Generar Código de Acceso'}</h4>
                    <button type="button" onClick={resetUserForm} className="p-1 text-slate-400 hover:text-slate-700 cursor-pointer">
                      <X className="w-5 h-5" />
                    </button>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`block w-full px-3 py-2 border rounded-xl text-xs focus:outline-none focus:ring-1 focus:ring-orange-500 text-slate-800 ${
                        isUserNameDuplicate ? 'bg-red-50 border-red-300 focus:ring-red-500' : 'bg-slate-50 border-slate-200'
                      }`}
                    />
                    {isUserNameDuplicate && (
                      <p className="text-[10px] text-red-600 font-semibold mt-1">Este nombre ya está en uso, elige otro</p>
                    )}
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">
                      {editUserId ? 'Código Nuevo (Opcional - dejar vacío para no cambiar)' : 'Código Único Auto-Generado'}
                    </label>
                    <div className="flex gap-2">
                      <input
                        type="text"
                        ...
                        required={!editUserId}
                        value={generatedCode}
                        placeholder={editUserId ? 'Opcional (Haz clic en Generar)' : 'Haz clic en Generar'}
                  ...
                  <div className="flex gap-2 border-t border-slate-100 pt-4">
                    ...
                    <button
                      type="submit"
                      disabled={savingUser || (!editUserId && !generatedCode) || isUserNameDuplicate}
                    ...
                    >
                      {savingUser ? 'Guardando...' : (editUserId ? 'Guardar Cambios' : 'Crear Acceso')}
                    </button>
                  </div>
                </form>
              </div>
