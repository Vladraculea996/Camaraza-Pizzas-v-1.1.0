...
export default function LoginView({ onLoginSuccess, onGoBack }: LoginViewProps) {
  ...
  const handleInitialSetup = async (e: FormEvent) => {
    ...
    try {
      // Check for name duplicates in database (case-insensitive and ignoring extra spaces)
      const existingUsers = await dbService.getItems<UsuarioAcceso>('usuarios_acceso');
      const normalizeName = (name: string) => name.trim().toLowerCase().replace(/\s+/g, ' ');
      const normalizedNewName = normalizeName(username);
      const nameExists = existingUsers.some(u => normalizeName(u.nombre) === normalizedNewName);
      if (nameExists) {
