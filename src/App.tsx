...
export default function App() {
  const [currentUser, setCurrentUser] = useState<{ nombre: string; rol: 'ceo' | 'staff' | 'vip' } | null>(() => {
    const saved = localStorage.getItem('camaraza_user_session');
    if (saved) {
      try {
  ...
  const [currentView, setCurrentView] = useState<'client' | 'kitchen' | 'admin' | 'login'>(() => {
    const savedUser = localStorage.getItem('camaraza_user_session');
    if (savedUser) {
      try {
        const user = JSON.parse(savedUser);
        if (user.rol === 'ceo') return 'admin';
        if (user.rol === 'staff') return 'kitchen';
        return 'client';
      } catch (e) {}
    }
    return 'client';
  ...
  const handleLoginSuccess = (user: { nombre: string; rol: 'ceo' | 'staff' | 'vip' }) => {
    setCurrentUser(user);
    localStorage.setItem('camaraza_user_session', JSON.stringify(user));
  ...
  const handleLogout = () => {
    setCurrentUser(null);
    localStorage.removeItem('camaraza_user_session');
    setCurrentView('client');
  };
